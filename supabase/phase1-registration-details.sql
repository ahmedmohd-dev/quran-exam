alter table public.student_registrations
  add column if not exists study_years integer not null default 0 check (study_years between 0 and 30),
  add column if not exists study_months integer not null default 0 check (study_months between 0 and 11);

drop function if exists public.register_student(uuid, text, integer, public.learning_level, text, text);
...
begin
  ...
  if not exists (select 1 from public.exam_periods where id = p_exam_period_id and status = 'registration_open') then raise exception 'Student registration is not open for this examination period.'; end if;
