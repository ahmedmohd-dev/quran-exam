...
begin
  ...
  if p_current_learning_level = 'quran' and p_current_learning_place !~ '^(?:[1-9]|[1-9][0-9]|10[0-9]|11[0-4])$' then raise exception 'Qur’an Surah number must be between 1 and 114.'; end if;
  if p_current_learning_level = 'alif' and p_current_learning_place !~ '^(?:[1-9]|1[0-9]|20)$' then raise exception 'Alif Fesel number must be between 1 and 20.'; end if;
