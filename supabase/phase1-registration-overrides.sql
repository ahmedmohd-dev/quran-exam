alter table public.exam_periods
  add column if not exists registration_override text not null default 'automatic';

alter table public.exam_periods
  drop constraint if exists exam_periods_registration_override_check;

alter table public.exam_periods
  add constraint exam_periods_registration_override_check
  check (registration_override in ('automatic', 'force_open', 'force_closed'));

update public.exam_periods
set registration_override = case when registration_force_open then 'force_open' else 'automatic' end;
