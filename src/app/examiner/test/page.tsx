...
function learningName(level: "alif" | "quran", place: string) {
  ...
  return level === "quran"
    ? surahs[number - 1] ?? place
    : alifFesels[number - 1] ?? place;
}

function markingAvailable(level: "alif" | "quran", place: string) {
  const number = Number(place);
  return (
    (level === "alif" && number >= 1 && number <= 27) ||
    (level === "quran" && number >= 1 && number <= 114)
  );
}
  ...
  useEffect(() => {
    async function loadStudents() {
      ...
      const { data: resultRows, error: resultsError } = assignmentIds.length
        ? await supabase
            .from("exam_results")
            .select("examiner_assignment_id, status, total_mark, result_class")
      ...
      const results = new Map(
        (resultRows ?? []).map((result) => [
          result.examiner_assignment_id,
          result as ResultSummary,
        ])
      );
      setStudents(
        rows.flatMap((row) => {
          ...
          return [
            {
              ...
              place: registration.current_learning_place ?? "",
  ...
  return (
    <ExaminerShell>
      <header className="examiner-header">
        <Link className="back-link" href="/examiner">
          ← ወደ ዋና ገጽ
        </Link>
        <p className="eyebrow">የፈተና መጀመሪያ</p>
        <h1>ፈተና ጀምር</h1>
        <p>የተመደቡልዎትን ተማሪዎች ይምረጡ።</p>
      </header>
      {message && <p className="admin-message">{message}</p>}
      <section className="examiner-list">
        {students.map((student, index) => (
          <article
            className="admin-card examiner-student"
          ...
          >
            ...
            <div>
              ...
              <p>
                ...
                {learningName(student.level, student.place)}
              </p>
              <small>
                የኡስታዝ ኮድ: <strong>{student.ustazCode}</strong>
              </small>
            </div>
            {student.result?.status === "submitted" && (
              <div className="examiner-result-summary">
                <strong>
                  {Number(student.result.total_mark).toFixed(2)} / 100
                </strong>
                <span>{rankLabel(student.result.result_class)}</span>
              </div>
            )}
            {markingAvailable(student.level, student.place) ? (
              <Link
                className="primary-button examiner-start-button"
                href={`/examiner/test/${student.assignmentId}`}
              >
                {student.result?.status === "submitted"
                  ? "ውጤት ለማስተካከል"
                  : student.result?.status === "draft"
                  ? "ፈተናውን ለመቀጠል"
                  : "ፈተና ጀምር"}
              </Link>
            ) : (
        ...
        {!message && students.length === 0 && (
          <section className="admin-card empty-state">
            <strong>ምንም ተመድቦ የለም</strong>
            <p>እባክዎ አስተዳዳሪውን ያነጋግሩ።</p>
          </section>
        )}
      </section>
    </ExaminerShell>
