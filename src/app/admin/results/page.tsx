...
export default function AdminResultsPage() {
  ...
  function exportUstazPdf(ustazName: string, selectedResults: Result[]) {
    const rows = selectedResults.map((result) => {
      const extra = supplemental.get(result.student_registration_id);
      const student = result.assignment?.student_registration?.student?.full_name ?? "Unknown student";
      return `<tr><td>${exportValue(student)}</td><td>${Number(result.total_mark).toFixed(2)} / 100</td><td>${exportValue(result.result_class)}</td><td>${exportValue(revisionName(result))}</td><td>${Number(extra?.hisnul_muslim_mark ?? 0).toFixed(2)} / 20</td><td>${Number(extra?.homework_mark ?? 0).toFixed(2)} / 5</td><td>${exportValue(result.examiner_comment ?? "")}</td></tr>`;
    }).join("");
    ...
    printWindow.document.write(`<html><head><title>${exportValue(ustazName)} — Results</title><style>body{font-family:Arial;padding:24px}h1{color:#176b4e;text-align:center}h2{text-align:center}table{border-collapse:collapse;width:100%;font-size:10px}th,td{border:1px solid #b8c8bf;padding:7px;text-align:left}th{background:#e8f3ed}@media print{button{display:none}}</style></head><body><h1>የቁርአን ክለሳ ፈተና</h1><h2>${exportValue(ustazName)} · የተማሪዎች ውጤት</h2>${`<table><thead><tr><th>Student</th><th>Qur'an</th><th>Rank</th><th>Revision</th><th>Hisnul Muslim</th><th>Homework</th><th>Comment</th></tr></thead><tbody>${rows}</tbody></table>`}<script>window.onload=function(){window.print();}</script></body></html>`);
    printWindow.document.close();
  }
