...
type Result = { id: string; examiner_assignment_id: string; status: "draft" | "submitted"; total_mark: number; result_class: string; revision_place: number | null; examiner_comment: string | null; examiner: { full_name: string } | null; assignment: { student_registration: { student: { full_name: string } | null } | null } | null };
...
export default function AdminResultsPage() {
  ...
  async function load() {
    ...
    if (!user) return;
    const { data, error } = await supabase.from("exam_results").select("id, examiner_assignment_id, status, total_mark, result_class, revision_place, examiner_comment, examiner:profiles!exam_results_examiner_id_fkey(full_name), assignment:examiner_assignments(student_registration:student_registrations(student:students(full_name)))").order("updated_at", { ascending: false });
